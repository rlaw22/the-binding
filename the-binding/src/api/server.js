...
async function createServer(options = {}) {
  ...
  // --- COST STATUS ENDPOINTS ---
  app.get('/api/sessions/:id/cost', async (request, reply) => {
    const data = sessions.get(request.params.id);
    if (!data) return reply.status(404).send({ error: 'Session not found' });
    const optimizer = data.costOptimizer;
    if (!optimizer) return { status: 'no_optimizer', message: 'Cost tracking not available for this session' };
    return optimizer.getCostSummary();
  });

  // Global cost stats (all sessions)
  app.get('/api/cost/stats', async () => {
    const allCosts = [];
    for (const [sid, data] of sessions) {
      if (data.costOptimizer) {
        allCosts.push(data.costOptimizer.getCostSummary());
      }
    }
    const totalCost = allCosts.reduce((sum, c) => sum + (c.totalCost || 0), 0);
    return {
      activeSessions: allCosts.length,
  ...
  app.post('/api/sessions', async (request, reply) => {
    ...
    if (ADMIN_KEY) {
      const access = AccessControl.checkAccess(tokenCode, gameMode || 'storyline');
    ...
    if (resolvedGameMode === 'digital_dm') {
      ...
      // Cost optimizer: smart model routing + budget tracking (skip in mock mode)
      const costOptimizer = createCostOptimizer({ llmConfig, sessionId: session.id });

      const game = createGame({
        adventureId: useSceneGraph ? scenarioId : 'digital_dm',
        adventureName,
        gameMode: 'digital_dm',
        llmProvider: llmConfig.mock ? createProvider(llmConfig) : costOptimizer.llm.bind(costOptimizer),
        ruleEngine: RuleEngine,
        diceService: DiceService
      });

      game._costOptimizer = costOptimizer;
      ...
      sessions.set(session.id, { session, game, coinPool: null, sceneCoins: [], currentSceneCoins: [], runningCoinTotal: 0, completedScenes: [], history: [], inventory: Inventory.createInventory(), difficulty: new DynamicDifficulty(), difficultyProfile: null, gameMode: 'digital_dm', modeConfig, costOptimizer });
    ...
    sessions.set(session.id, { session, game, coinPool, sceneCoins: [], currentSceneCoins: [], runningCoinTotal: 0, completedScenes: [], history: [], inventory: Inventory.createInventory(), difficulty: new DynamicDifficulty(), difficultyProfile: advDiffProfile, gameMode: resolvedGameMode, modeConfig, costOptimizer });
    rejoinCodes.set(rejoinCode, session.id);
    if (tokenCode) TokenStore.recordSession(tokenCode);
    session._rejoinCode = rejoinCode;
    markDirty();
    ...
    if (startInfo) {
      ...
      // Narrative prologue — establishes who the player is and why they're here
      const prologueText = getPrologue(adventureId, player.character.name, player.character.class);
      if (prologueText) {
        recordMessage(session.id, MessageRouter.narration(prologueText, {}));
        addTurn(game.contextManager, 'assistant', prologueText);
      }
  ...
  app.post('/api/sessions/:id/actions', async (request, reply) => {
    ...
    try {
      ...
      // Cost optimization: check TTS cache first, skip if session budget exceeded
      if (voiceEnabled) {
        const optimizer = data.costOptimizer || game._costOptimizer;
        const ttsCacheHit = optimizer ? optimizer.ttsCache.get(result.narrative) : null;
        if (ttsCacheHit) {
          recordMessage(sessionId, MessageRouter.voiceAudio(ttsCacheHit.taskId || 'cached', ttsCacheHit.status || 'cached'));
        } else if (!optimizer || optimizer.canGenerateTTS()) {
          voiceService.generate(result.narrative).then(ttsResult => {
            if (ttsResult.taskId) {
              ...
              if (optimizer) optimizer.ttsCache.set(result.narrative, ttsResult);
            }
          }).catch(err => {
      ...
      // Cost optimization: skip image generation if session budget exceeded
      if (result.combat && result.combat.active && !result.combat.existing) {
        const imgBudget = data.costOptimizer ? data.costOptimizer.canGenerateImage() : { allowed: true };
        if (!imgBudget.allowed) {
          console.log(`[Cost] Skipping combat image — ${imgBudget.reason}`);
        } else {
        try {
