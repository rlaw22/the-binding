...
assert.ok(bridge.includes('Reading history. Gameplay is paused.'));
assert.ok(bridge.includes('Returned to present.'));
assert.ok(bridge.includes('previousFocus'));
...
assert.ok(bridge.includes('submissionInFlight'));
assert.ok(bridge.includes('.finally(function ()'));
assert.ok(bridge.includes('var previousFocus = null;'));
